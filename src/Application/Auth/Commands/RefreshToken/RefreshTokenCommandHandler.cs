...
public sealed class RefreshTokenCommandHandler : IRequestHandler<RefreshTokenCommand, Result<AuthResponse>>
{
    ...
    public async Task<Result<AuthResponse>> Handle(RefreshTokenCommand request, CancellationToken cancellationToken)
    {
        ...
        if (storedToken is null)
            return new Result<AuthResponse>(DomainErrors.Auth.RefreshTokenNotFound);

        if (storedToken.IsRevoked)
            return new Result<AuthResponse>(DomainErrors.Auth.RefreshTokenRevoked);

        if (storedToken.IsUsed)
            return new Result<AuthResponse>(DomainErrors.Auth.RefreshTokenAlreadyUsed);

        if (DateTime.UtcNow >= storedToken.ExpiresAt)
            return new Result<AuthResponse>(DomainErrors.Auth.RefreshTokenExpired);
